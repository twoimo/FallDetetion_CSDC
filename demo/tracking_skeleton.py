...
from BoTSORT.yolov7.utils.plots import plot_one_box
...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            all_results.append((online_ids, online_tlwhs, online_cls))
    ...
    for result in all_results:
        ids, bboxes, cls = result
        result_per_index = []
        for i in range(len(ids)):
            result_per_index.append([ids[i]] + list(bboxes[i]) + [cls[i]])
        modified_results.append(result_per_index)

    person_bboxes = [[] for _ in range(len(modified_results))]
    face_bboxes = [[] for _ in range(len(modified_results))]

    for idx, result in enumerate(modified_results):
        for obj in result:
            if obj[5] == 0.0:
                person_bboxes[idx].append(np.array(obj[1:5]))
            elif obj[5] == 1.0:
                face_bboxes[idx].append(np.array(obj[1:5]))
    

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--weights', nargs='+', type=str, default='yolov7_custom.pt', help='model.pt path(s)')
    parser.add_argument('--source', type=str, default='test_data/Prison_Fall_1.avi', help='source')  # file/folder, 0 for webcam
...
def parse_args():
    ...
    parser.add_argument('--out_filename', default='demo/Prison_Fall_1.avi', help='output filename')
    parser.add_argument(
        '--pose-config',
...
def main():
    ...
    frame_paths, frames = frame_extract(source, tmp_dir.name)
